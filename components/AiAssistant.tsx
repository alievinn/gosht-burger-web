...
import { X, Send, Loader2, Sparkles, Trash2, MessageSquare } from 'lucide-react';
...
export const AiAssistant: React.FC<AiAssistantProps> = ({ onAddToCart }) => {
  const [isOpen, setIsOpen] = useState(false);
  const [input, setInput] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [messages, setMessages] = useState([
    { role: 'assistant', text: 'Merhaba! Ben **Gosht Gurme Asistan**. Size özel burger önerileri sunabilirim. Bugün ne yemek istersiniz?' }
  ]);
    ...
    try {
      const response = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 500,
          system: 'Sen Gosht Burger restoranının AI asistanısın. Batman şehrinde premium burger restoranısın. Menüde burgerler, steak ve çocuk menüsü var. Kısa, samimi ve Türkçe cevaplar ver.',
          messages: [
      ...
      const text = data.content?.[0]?.text || 'Üzgünüm, yanıt veremiyorum.';
    ...
    <>
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <div className="p-4 flex justify-between items-center border-b border-stone-800 bg-stone-900">
              ...
              <div className="flex gap-2">
                <button onClick={() => setMessages([{ role: 'assistant', text: 'Merhaba! Size nasıl yardımcı olabilirim?' }])} className="p-2 text-stone-400 hover:text-red-500 transition-colors">
                  <Trash2 size={18} />
                </button>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {messages.map((msg, idx) => (
                <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[80%] p-3 rounded-2xl text-sm ${msg.role === 'user' ? 'bg-red-800 text-white' : 'bg-stone-800 text-stone-200'}`}>
                    {msg.text}
                  </div>
                </div>
              ))}
